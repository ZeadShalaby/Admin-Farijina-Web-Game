...
@section('css')
    ...
    <style>
        ...
        body {
            background: linear-gradient(180deg, var(--bg), #f4f6fa 60%);
            direction: rtl;
        }
        ...
        .btn-primary {
            ...
            color: #fff;
            box-shadow: 0 6px 14px rgba(31, 111, 235, 0.16);
        }
        ...
        .perm-card {
            ...
            border: 1px solid var(--border);
            padding: 10px;
            background: linear-gradient(90deg, #ffffff, #fcfdff);
        }
                                                ...
                                                <div style="display:flex;align-items:center;gap:12px">
                                                    <label class="select-all">
                                                        <input type="checkbox" class="select-group"
                                                            data-target="{{ \Illuminate\Support\Str::slug($groupName) }}" />
                                                        <span>تحديد الكل</span>
                                                    </label>
                                                    <button type="button" class="btn btn-ghost"
                                                        style="padding:6px 8px;border-radius:8px;"
                                                        onclick="toggleGroup(this)">⌄</button>
                                                </div>
                                            </div>
                                            ...
                                            <div class="perm-row">
                                                @foreach ($groupPermissions as $permission)
                                                    <label class="checkbox" style="min-width:140px;">
                                                        <input style="color: #1b2b3a" type="checkbox" name="permissions[]"
                                                            value="{{ $permission->id }}"
                        ...
                        <div class="section">
                            ...
                            <div class="meta-card" style="margin-top:6px;">
                                <div style="display:flex;gap:12px;align-items:center">
                                    <div>
                                        <div class="avatar" id="roleAvatar">
                                            ...
                                            @else
                                                <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64'><rect width='100%' height='100%' fill='%23dfe7f7'/><text x='50%' y='55%' font-size='28' text-anchor='middle' fill='%23576b95' font-family='Arial' dy='.35em'>{{ strtoupper(substr($role->name, 0, 1)) }}</text></svg>"
                                                    alt="avatar">
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div style="margin-top:12px; display:flex;flex-direction:column; gap:8px;">
                                    <div style="font-size:13px;color:var(--muted)"><strong>عدد الصلاحيات:</strong>
                                        {{ $rolePermissions ? count($rolePermissions) : 0 }}</div>
                                    <div style="font-size:13px;color:var(--muted)"><strong>مُنشأ في:</strong>
                                        {{ $role->created_at ? $role->created_at->format('Y-m-d') : '-' }}</div>
                                </div>
                            </div>
                            ...
                            <div class="section" style="margin-top:12px;">
                                ...
                                <div style="display:flex; flex-direction:column; gap:8px;">
                                    <a href="{{ route('rules.index') }}" class="btn btn-ghost"
                                        style="text-align:center">الرجوع للقائمة</a>
                                </div>
                            </div>
                        </div>
                    </div>
